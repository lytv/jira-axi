...
export function toAdf(
  value: string | AdfDocument | undefined,
  ...
  for (const block of value.replace(/\r\n/g, "\n").split(/\n\n+/)) {
    ...
    content.push({
      ...
      content: lines.flatMap((line, index) =>
        index ? [{ type: "hardBreak" }, ...inline(line)] : inline(line),
      ),
    });
  }
  ...
  const read = (node: AdfNode): string => {
    ...
    if (node.type === "orderedList") {
      const order =
        ...
        Number.isFinite(node.attrs.order)
          ? node.attrs.order
          : 1;
      return (node.content ?? [])
        .map(
          (item, index) => `${order + index}. ${read(item).replace(/^- /, "")}`,
        )
        .join("\n");
    }
    return text;

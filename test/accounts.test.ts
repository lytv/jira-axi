...
describe("accounts", () => {
  it("resolves explicit, single, then default accounts", () => {
    ...
    expect(resolveAccount([account("one"), account("two", true)]).id).toBe(
      "two",
    );
    expect(
      resolveAccount([account("one"), account("two", true)], "one").id,
    ).toBe("one");
  });
  it("rejects a multi-account lookup without a default with exit code 2", () => {
    try {
      resolveAccount([account("one"), account("two")]);
    } catch (error) {
  ...
  it("imports jira-cli sites without inventing token values or a default for multiple sites", async () => {
    ...
    const fixture = join(directory, "config.yml");
    await writeFile(
      fixture,
    ...
    expect(
      imported.every(
        (item) =>
          item.tokenSource.kind === "jira-cli-ref" &&
          item.tokenSource.ref === "JIRA_API_TOKEN",
      ),
    ).toBe(true);
  ...
  it("rejects an OAuth acli credential", async () => {
    ...
    await expect(importAccounts("acli", fixture, [])).rejects.toThrow(
      "OAuth credentials",
    );
  });
